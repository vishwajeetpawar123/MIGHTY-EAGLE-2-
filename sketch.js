...
function draw() {
  ...
  textAlign(CENTER);
    ...
    text("BECOUSE OF RADIATION,FOOD,AND SOME CAN'T EVEN SEE THE BEAUTIFUL WORLD",windowWidth/2-100,windowHeight/2+100)
  ...
  if(gameState===4){
    ...
    text(" 2}stay protected from the radaition ions swinging through th airto hit you ",windowWidth/2-50,windowHeight/2-160);
	text(" try not to reach 8 hits or you won't make it to the mighty eagle.",windowWidth/2-50,windowHeight/2-120);

   
   
   
    text("3]there are radiation ions through the air try to avoid them.",windowWidth/2-50,windowHeight/2-80);

    text("4]there is a tower nearby which is realising the ions time to time.",windowWidth/2-50,windowHeight/2-30);

    
    
    
    text("6]to get knowledge and slogens about the birds win and also try losing.",windowWidth/2-50,windowHeight/2);




    text("7]reload the page to get to the home page.",windowWidth/2-50,windowHeight/2+30);
